...
\texttt{parts/strips.sty} has \verb`\strips[options]` macro, which produces a formal definition of classical planning.
...
\begin{itemize}
 ...
 \item \texttt{satisficing} (adds a note about satisficing search. Implies \texttt{optimal}),
 \item \texttt{agile} (adds a note about agile search. Implies \texttt{satisficing})
...
\verb`\heuristics[options]` macro describes heuristic functions and introduce several heuristics
specifies in the options.
Available options are:
\begin{itemize}
...
Use it like \verb`\heuristics` \texttt{[ad=2,ff]}.
