...
\begin{itemize}
 ...
 \item \texttt{transition-only} (removes the detail about action applications for compressing papers that focus on search).
\end{itemize}
...
\begin{itemize}
 ...
 \item \texttt{simplified}: shorten the description of heuristics.
\end{itemize}
...
\begin{itemize}
 ...
 \item \texttt{hmax} ($\hmax$, max heuristics)
 \item \texttt{ff} ($\ff$, FF heuristics)
 \item \texttt{gc} ($\gc$, goal count heuristics)
 \item \texttt{cg} ($\gc$, causal graph heuristics)
 \item \texttt{cea} ($\ce$, context enhanced additive heuristics)
\end{itemize}
...
\heuristicusage{simplified}

\heuristicusage{hmax,ad,ff,gc,cg,cea}
